...
### Posthoc: Concurrent NEO Total grooming drop1 analysis
```{r echo= FALSE}
library(lmerTest)
```
...
Posthoc model 1
```{r}
NEO.Con.LMM.TotalGr.drop1model=lmer(log(NEO.CR)~ FreqTotalGrJuv + FreqTotalGrAM + FreqGRAF + RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) +  (1|ID) + (1|Behav_Date), data=Concurrent_NEO)
```

```{r}
#model diagnostics
TotalGr.drop1 <- vif(NEO.Con.LMM.TotalGr.drop1model)
write.csv(TotalGr.drop1, "vif.TotalGr.drop1.csv")

pdf("TotalGr.drop1.frplot.pdf")
plot(fitted(NEO.Con.LMM.TotalGr.drop1model), resid(NEO.Con.LMM.TotalGr.drop1model),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=4.7, y=1.3, labels="(A)", xpd = NA, cex=1.5, font=2)
...
pdf("TotalGr.drop1.qqplot.pdf")
qqnorm(resid(NEO.Con.LMM.TotalGr.drop1model))
text(x=-3.6, y=1.3, labels="(B)", xpd = NA, cex=1.5, font=2)
dev.off()
```
...
dimnames(NEO.Con.LMM.TotalGr.drop1test)[[1]] <- list("Total Grooming with Juveniles", "Total Grooming with Adult Males", "Total Grooming with Adult Females", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)")
...
NEO.Con.LMM.TotalGr.drop1test.tidy <- NEO.Con.LMM.TotalGr.drop1test %>%
  ...
  save_kable(file= "TotalGr.drop1.html"); NEO.Con.LMM.TotalGr.drop1test.tidy
```
...
NEOCONtotalGr.plot <- ggplot(x, aes(FreqTotalGR, fit)) + 
  ...
  geom_errorbar(aes(ymin=fit-se,
                    ymax=fit+se), color="grey37", alpha=0.2) +
  ...
  geom_point(data= Concurrent_NEO, aes(x= FreqTotalGR, y= log(NEO.CR)), size=0.2)
...
Fig3.Obj3 <- annotate_figure(Fig3.Obj3, bottom = text_grob(expression(paste("* p< 0.05; ** p<0.01; *** p<0.001 ", "\u2020", " log transformed")), hjust = 2.7, x = 1, face = "italic", size = 8))
