...
x<-c("car", "lme4", "effects", "ggplot2", "grid", "gridExtra", "dplyr", "tibble", "stringr", "condformat", 'kableExtra', 'crayon', 'sjPlot', "tidyverse", "ggpubr", "cowplot", "lubridate", "hms", "chron")
...
```{r}
BehaviorOnly <- read.csv("RhesusFullBehavior.csv")
...
## IOR
```{r}
GroomingIOR <- read.csv("GroomingIOR.csv")
ScanIOR <- read.csv("ScanIOR.csv")
```
...
```{r}
GC_Behavior_raw=read.csv("GC_behavior_matched.csv", h=T) 
...
```{r}
GCcorSoc <- p.adjust(c(AnovaGC.LMM1[2,3], AnovaGC.LMM2[2,3], AnovaGC.LMM3[2,3], AnovaGC.LMM4[2,3]), method = "hommel"); GCcorSoc
```
...
tab_model(NEO.TL.LMM3, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Receiving Grooming", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Time-lagged uNEO (log transformed)", file="NEO.TL.LMM3.html")
...
tab_model(NEO.TL.LMM4, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Giving Grooming", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Time-lagged uNEO (log transformed)", file="NEO.TL.LMM4.html")
...
```{r}
TLNEOcorSoc <- p.adjust(c(AnovaNEO.TL.LMM1[2,3], AnovaNEO.TL.LMM2[2,3], AnovaNEO.TL.LMM3[2,3], AnovaNEO.TL.LMM4[2,3]), method = "hommel"); TLNEOcorSoc
```
...
tab_model(NEO.Con.LMM1, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Proximity", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM1.html")


#model comparison
NEO.Con.LMM1.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered +I(DateDiffCentered ^ 2) + (1|ID) + (1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM1, NEO.Con.LMM1.red) # Model with proximity not sig better fit than model without
```
...
plot(fitted(NEO.Con.LMM2), resid(NEO.Con.LMM2),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=4.7, y=1.2, labels="(A)", xpd = NA, cex=1.5, font=2)
dev.off()

pdf("NEO.Con.LMM2.qqplot.pdf")
qqnorm(resid(NEO.Con.LMM2))
text(x=-3.6, y=1.2, labels="(B)", xpd = NA, cex=1.5, font=2)
dev.off()

#model summary
summary(NEO.Con.LMM2)
AnovaNEO.Con.LMM2 <- Anova(NEO.Con.LMM2, type= "III")
tab_model(NEO.Con.LMM2, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Total Grooming", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM2.html")

#model comparison
NEO.Con.LMM2.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM2, NEO.Con.LMM2.red) #model with total grooming IS sig better fit than model without total grooming
```
...
tab_model(NEO.Con.LMM3, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Receiving Grooming", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM3.html")

#model comparison
NEO.Con.LMM3.red=lmer(log(NEO.CR)~  RelRank + DateDiffCentered +I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM3, NEO.Con.LMM3.red) #model with receiving grooming IS sig better fit than model without
```
...
tab_model(NEO.Con.LMM4, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Giving Grooming", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM4.html")

#model comparison
NEO.Con.LMM4.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM4, NEO.Con.LMM4.red) #model with giving grooming not sig better fit than model without
```

p adjust for 4 Obj 1 models with concurrent uNEO as response variable
```{r}
ConNEOcorSoc <- p.adjust(c(AnovaNEO.Con.LMM1[2,3], AnovaNEO.Con.LMM2[2,3], AnovaNEO.Con.LMM3[2,3], AnovaNEO.Con.LMM4[2,3]), method = "hommel"); ConNEOcorSoc
```
...
```{r}
GCcorSex <- p.adjust(c(AnovaGC.LMM5[2,3], AnovaGC.LMM6[2,3]), method = "hommel"); GCcorSex
```
...
tab_model(NEO.TL.LMM5, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Copulation", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Time-lagged uNEO (log transformed)", file="NEO.TL.LMM5.html")

#model comparison
NEO.TL.LMM5.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) + (1|Behav_Date), data=TL_NEO)
anova(NEO.TL.LMM5, NEO.TL.LMM5.red) #model including copulation is not sig better fit than model without
```
...
sjPlot::tab_model(NEO.TL.LMM6, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Consortship", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Time-lagged uNEO (log transformed)", file="NEO.TL.LMM6.html")


#model comparison
NEO.TL.LMM6.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=TL_NEO)
anova(NEO.TL.LMM6, NEO.TL.LMM6.red) #model including consortship not sig better fit than model without
```

p adjust for 2 Obj 2 models with time-lagged NEO as response variable
```{r}
TLNEOcorSex <- p.adjust(c(AnovaNEO.TL.LMM5[2,3], AnovaNEO.TL.LMM6[2,3]), method = "hommel"); TLNEOcorSex
```
...
tab_model(NEO.Con.LMM5, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Copulation", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM5.html")

#model comparison
NEO.Con.LMM5.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM5, NEO.Con.LMM5.red) #model including copulation IS sig better fit than model without
```
...
tab_model(NEO.Con.LMM6, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Consortship", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)"), dv.labels= "Response: Concurrent uNEO (log transformed)", file="NEO.Con.LMM6.html")

#model comparison
NEO.Con.LMM6.red=lmer(log(NEO.CR)~ RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) +(1|Behav_Date), data=Concurrent_NEO)
anova(NEO.Con.LMM6, NEO.Con.LMM6.red) #model including consortship is not sig better fit than model without
```

p adjust for 2 Obj 2 models with concurrent uNEO as response variable
```{r}
ConNEOcorSex <- p.adjust(c(AnovaNEO.Con.LMM5[2,3], AnovaNEO.Con.LMM6[2,3]), method = "hommel"); ConNEOcorSex
```
...
Posthoc model 
```{r}
NEO.Con.LMM.RecGr.drop1model=lmer(log(NEO.CR)~ FreqGrRecJuv + FreqGrRecAM + FreqGrRecAF + RelRank + DateDiffCentered + I(DateDiffCentered ^ 2) + (1|ID) + (1|Behav_Date), data=Concurrent_NEO)
```

```{r}
#model diagnostics
RecGr.drop1 <- vif(NEO.Con.LMM.RecGr.drop1model)
write.csv(RecGr.drop1, "vif.RecGr.drop1.csv")

pdf("RecGr.drop1.frplot.pdf")
plot(fitted(NEO.Con.LMM.RecGr.drop1model), resid(NEO.Con.LMM.RecGr.drop1model),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=4.7, y=1, labels="(A)", xpd = NA, cex=1.5, font=2)
dev.off()

pdf("RecGr.drop1.qqplot.pdf")
qqnorm(resid(NEO.Con.LMM.RecGr.drop1model))
text(x=-3.6, y=1, labels="(B)", xpd = NA, cex=1.5, font=2)
dev.off()
```


```{r}
NEO.Con.LMM.RecGr.drop1test <- as.data.frame(drop1(NEO.Con.LMM.RecGr.drop1model, test= "Chi")); NEO.Con.LMM.RecGr.drop1test


dimnames(NEO.Con.LMM.RecGr.drop1test)[[1]] <- list("Receiving Grooming from Juveniles", "Receiving Grooming from Adult Males", "Receiving Grooming from Adult Females", "Dominance rank", "Latency to sample collection (linear)", "Latency to sample collection (quadratic)")


NEO.Con.LMM.RecGr.drop1test.tidy <- NEO.Con.LMM.RecGr.drop1test %>%
  rownames_to_column(var= "Predictor") %>%
  rename("p" = "Pr(>F)") %>%
  rename("f-statistic" = "F value") %>%
  rename("Sum of Squares" = "Sum Sq") %>%
  rename("Mean Square" = "Mean Sq") %>%
  rename("df (numerator)"= "NumDF") %>%
  rename("df (denominator)"= "DenDF") %>%
  mutate_if(is.numeric, round, 3) %>%
  mutate("p" = cell_spec(p, "html", bold = (p < 0.05))) %>%
  kable(format="html", escape = F) %>%
  row_spec(0, italic=T) %>%
  kable_styling(full_width=TRUE) %>%
  save_kable(file= "RecGr.drop1.html"); NEO.Con.LMM.RecGr.drop1test.tidy
```

# OBJECTIVE 3

```{r}
GCunmatched <- read.csv("GCunmatched.csv")

NEOunmatched <- read.csv("NEOunmatched.csv")

```

## LMM changes in fGC over time
...
plot(fitted(GC.month.mod), resid(GC.month.mod),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=6.02, y=1.3, labels="(A)", xpd = NA, cex=1.5, font=2)
dev.off()

pdf("GC.month.qqplot.pdf")
qqnorm(resid(GC.month.mod))
text(x=-3.5, y=1.3, labels="(B)", xpd = NA, cex=1.5, font=2)
dev.off()

#model summary
summary(GC.month.mod)
AnovaGC.Month <- Anova(GC.month.mod, type= "III")
tab_model(GC.month.mod, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Month", "Dominance rank"), dv.labels= "Response: fGC (log transformed)", file="GC.month.mod.html")
...
plot(fitted(NEO.month.mod), resid(NEO.month.mod),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=4.6, y=1.3, labels="(A)", xpd = NA, cex=1.5, font=2)
dev.off()
...
AnovaNEO.Month <- Anova(NEO.month.mod, type= "III")
tab_model(NEO.month.mod, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Month", "Dominance rank"), dv.labels= "Response: uNEO (log transformed)", file="NEO.month.mod.html")
...
#model summary
summary(Cr.month.mod) #month sig predictor of creatinine
Anova(Cr.month.mod)
...
## Model change in uNEO over time, excluding data from January 
...
plot(fitted(NEOnoJanmod), resid(NEOnoJanmod),
    main = "Fitted vs. Residual plot",
     xlab= "Residual values",
     ylab= "Fitted values")
text(x=4.65, y=1.3, labels="(A)", xpd = NA, cex=1.5, font=2)
dev.off()
...
tab_model(NEOnoJanmod, show.df= TRUE, show.stat=TRUE, show.se= TRUE, show.ci=FALSE, string.se = "SE",  string.stat = "t-value", p.val = "kr", pred.labels= c("(Intercept)", "Month", "Dominance rank"), dv.labels= "Response: uNEO excluding January (log transformed)", file="NEOnoJanmod.html")

#comparison to reduced model
NEOnoJanmod.red <- lmer(log(NEO.CR) ~ RelRank + (1 |Animal.ID), data= NEOnoJan)
anova(NEOnoJanmod, NEOnoJanmod.red, type= "III") #model including month IS sig better fit than model without
```
...
NEOMonthModsCor <- p.adjust(c(AnovaNEO.Month[2,3], AnovaNEO.MonthNOJAN[2,3]), method= "hommel")
...
Obj1.Summary.Kable <- Obj1.Summary %>%
  ...
  rename("P-value"= raw_P_value, "adj P-value**" = P_value, "Wald's χ2" = Walds_Chi_square, "t-value" = t_value, "Std. Error"= Std_Error, "Fixed Effect*"= Fixed_Effect, "Response Variable"= Response_variable) %>%
  kable(format="html", escape = F, align='llcccccc') %>%
...
### Obj 3 table
```{r echo=FALSE}
Obj3.Summary <-data.frame(Response_variable = c("fGC (n=195)", "uNEO (n=324)", "uNEO without January data (n=300)"), Fixed_Effect = c("Month", "Month", "Month"), "Estimate" = c(summary(GC.month.mod)$coef[2,"Estimate"], summary(NEO.month.mod)$coef[2,"Estimate"],  summary(NEOnoJanmod)$coef[2,"Estimate"]), Std_Error= c(summary(GC.month.mod)$coef[2, "Std. Error"], summary(NEO.month.mod)$coef[2, "Std. Error"], summary(NEOnoJanmod)$coef[2,"Std. Error"]), t_value= c(summary(GC.month.mod)$coef[2, "t value"], summary(NEO.month.mod)$coef[2, "t value"], summary(NEOnoJanmod)$coef[2,"t value"]), Walds_Chi_square= c(AnovaGC.Month[2,1], AnovaNEO.Month[2,1], AnovaNEO.MonthNOJAN[2,1]), raw_P_value= c(round(AnovaGC.Month[2,3], digits=3),"<0.001", "<0.001"), P_value = c(round(AnovaGC.Month[2,3], digits= 3), "<0.001", "<0.001"))
```
...
NEOCONGrRec.plot <- ggplot(data=x, aes(x=FreqGRRec, y=fit)) + 
  ...
  geom_errorbar(aes(ymin=fit-se,
                    ymax=fit+se), colour="grey37", alpha=0.2) +
  ...
  geom_point(data= Concurrent_NEO, aes(x= FreqGRRec, y= log(NEO.CR)), size=0.2)

NEO.Con.LMM3.Ests<-plot_model(NEO.Con.LMM3, show.values=TRUE, value.size=3, dot.size = 1, line.size = 0.5, colors=c("gray44", "black"), vline.color= "black", show.p=TRUE, rm.terms= c("DateDiffCentered", "I(DateDiffCentered^2)"), title="", axis.labels=c("Rank", "Receiving\n Grooming")) 


Fig1.Obj1.2 <- ggarrange(NEOCONGrRec.plot, NEO.Con.LMM3.Ests, nrow= 1, ncol= 2, labels= c("(a)", "(b)"), font.label = list(size = 8)) 


Fig1.Obj1.2 <- annotate_figure(Fig1.Obj1.2, bottom = text_grob(expression(paste("* p< 0.05; ** p<0.01; *** p<0.001 ", "\u2020", " log transformed")), hjust = 2.72, x = 1, face = "italic", size = 8)); Fig1.Obj1.2

ggsave("~/Google Drive/Projects/Masters/rhesus-sociality-fGC-uNEO/Fig1.pdf", Fig1.Obj1.2, width= 174, height= 100, units= "mm", device=cairo_pdf)
```
...
NEOCONFreqCop.plot <- ggplot(data=x, aes(x=FreqTimeSpentCop, y=fit)) + 
  ...
  theme(axis.title = element_text(size=6.5, color="black"),
        axis.title.y= element_text(size=8)) +
    geom_point(data= Concurrent_NEO, aes(x= FreqTimeSpentCop, y= log(NEO.CR)), size=0.2)


NEO.Con.LMM5.Ests<-plot_model(NEO.Con.LMM5, show.values=TRUE, title="", colors=c("black", "gray44"), rm.terms= c("DateDiffCentered", "I(DateDiffCentered^2)"), axis.labels=c("Rank", "Copulation"), dot.size = 1, vline.color= "black", value.size=3, show.p=TRUE, line.size = 0.5, group.terms=c(1,2))


Fig2.Obj2 <- ggarrange(NEOCONFreqCop.plot, NEO.Con.LMM5.Ests, labels= c("(a)", "(b)"), font.label = list(size = 8)) 
...
NEOMonth.plot <- ggplot(data=x, aes(x=Month, y=fit)) + 
  ...
  geom_point(size=0.3) +
  geom_errorbar(aes(ymin=fit-se,
                    ymax=fit+se), width= 0.0001, colour="grey37", alpha=0.2) +
  scale_x_continuous(limits=c(1,4)) +
  labs(y=expression(paste("uNEO (ng/mg CR)"^"\u2020")), x="Month") +
  theme(axis.title = element_text(size=6.5, color="black"),
        axis.title.y= element_text(size=8)) +
   geom_point(data= NEO, aes(x= Month, y= log(NEO.CR)), size=0.2)


NEOMonth.Ests<-plot_model(NEOmod, show.values=TRUE, title="", colors=c("black", "gray44"), axis.labels=c("Rank", "Month"), value.size=3, dot.size = 1, vline.color= "black", line.size = 0.5, group.terms=c(1,2)) 


Fig3.Obj3 <- ggarrange(NEOMonth.plot, NEOMonth.Ests, labels= c("(a)", "(b)"), font.label = list(size = 8)) 

Fig3.Obj3 <- annotate_figure(Fig3.Obj3, bottom = text_grob(expression(paste("* p< 0.05; ** p<0.01; *** p<0.001 ", "\u2020", " log transformed")), hjust = 2.72, x = 1, face = "italic", size = 8))
