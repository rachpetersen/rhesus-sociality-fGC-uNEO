...
x<-c("car", "lme4", "effects", "ggplot2", "grid", "gridExtra", "dplyr", "tibble", "stringr", "condformat", 'kableExtra', 'crayon', 'fitdistrplus', 'sjPlot', "tidyverse", "ggpubr", "cowplot", "lubridate", "hms")
...
mod1 <- lmer(log(CR.DF) ~ Month + (1+ RelRank|Animal.ID), data= NEO)
...
```{r}
NEOunmatched<-read.csv("~/Google Drive/Projects/Masters/rhesus-sociality-fGC-uNEO/NEOunmatched.csv", h=T)

NEOunmatched <- NEOunmatched %>%
  mutate(NEO_SampleNo = SampleNo) %>%
  select(NEO_SampleNo, Time)
  
TL_NEO<-read.csv("~/Google Drive/Projects/Masters/R input sheets/Time-lagged_NEO.csv", h=T)

TL_NEO <- left_join(TL_NEO, NEOunmatched)
TL_NEO$Time <- as.POSIXct(TL_NEO$Time,format="%I:%M %p")
TL_NEO$Time <- as.hms(TL_NEO$Time)
```
...
## SECTION 4.5: Mixed model for mating season progression
```{r}
...
NEO$Time <- as.POSIXct(NEO$Time,format="%I:%M %p")
NEO$Time <- as.hms(NEO$Time)
```
...
NEOdatemod <- lmer(log(NEO.CR) ~ Date + RelRank + (1 |Focal.ID), data= NEO)
summary(NEOdatemod)
